...
v) After that is done, type in ```esptool.py --chip esp32 --port /dev/ttyUSB0 write_flash -z 0x1000 (firmware file name)``` if you are a mac user and ```esptool.py --chip esp32 --port COM(port number) write_flash -z 0x1000 (firmware file name, need to path if ti)``` 


<!--stackedit_data:
eyJoaXN0b3J5IjpbNDAwMzcwODUsLTYwNTkxMDEyLDE0NjMwMT
Q2NDhdfQ==
-->